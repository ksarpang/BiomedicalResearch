---
title: "Project 2 *k*-means in Local Spark environment"
...
```{r read}
# read data in as CSV
pam50data_local <- spark_read_csv(sc, "pam50data_local", "/user/sarpankn/pam50_samples_t_w_type_3.csv")
pam50data_local %>% count()
pam50data_local
```
```{r cancer_type}
## List number of samples by Cancer Type
pam50data_local %>% group_by(CancerType) %>% summarize(count=n())
```
```{r Known_labels}
## Create a dataset of samples with known cancer types only
pam50data_local1 <- pam50data_local %>% filter(CancerType %in% c('Basal','Her2','LumA', 'LumB','Normal'))
pam50data_local1
colnames(pam50data_local1)
```
```{r Spark_kmeans_predictions5}
# Train the k-means model on known labels and generate predictions using  clusters
spark_kmeans5 <- ml_kmeans(pam50data_local1, ~ . - SampleID - CancerType, centers = 5)
spark_predict5 <- ml_predict(spark_kmeans5, pam50data_local)
...
# Train the k-means model on known labels and generate predictions using  clusters
spark_kmeans6 <- ml_kmeans(pam50data_local1, ~ . - SampleID - CancerType, centers = 6)
spark_predict6 <- sdf_predict(spark_kmeans6, pam50data_local)
...
# Train the k-means model on known labels and generate predictions using  clusters
spark_kmeans7 <- ml_kmeans(pam50data_local1, ~ . - SampleID - CancerType, centers = 7)
spark_predict7 <- sdf_predict(spark_kmeans7, pam50data_local)
...
# Train the k-means model on known labels and generate predictions using  clusters
spark_kmeans8 <- ml_kmeans(pam50data_local1, ~ . - SampleID - CancerType, centers = 8)
spark_predict8 <- sdf_predict(spark_kmeans8, pam50data_local)
...
# Train the k-means model on known labels and generate predictions using  clusters
spark_kmeans9 <- ml_kmeans(pam50data_local1, ~ . - SampleID - CancerType, centers = 9)
spark_predict9 <- sdf_predict(spark_kmeans9, pam50data_local)
...
# Train the k-means model on known labels and generate predictions using  clusters
spark_kmeans10 <- ml_kmeans(pam50data_local1, ~ . - SampleID - CancerType, centers = 10)
spark_predict10 <- sdf_predict(spark_kmeans10, pam50data_local)
...
# Iteration k = 8, has better clustering result because it has the lowest misclassification rate among all "k values" 
# Reassigning NAs to the respective cluster identities as done by majority vote thereof.